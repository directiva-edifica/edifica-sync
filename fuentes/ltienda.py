"""
Fuente: LTIENDA (LG Uruguay oficial, plataforma Fenicio, comercio 'lguy')
Marca LG. Precios ya en USD (sin conversion ni margen). Todo el catalogo.
"""
import requests, re
...
NOMBRE = "ltienda"
FEED_URL = "https://www.ltienda.com.uy/feeds/productos/lguy/fenicio"
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; EdificaBot/1.0)"}
NS = {"a": "http://www.w3.org/2005/Atom"}

MAPEO = {
    "Televisores > OLED": ["Electro", "TV"],
    "Televisores > QNED": ["Electro", "TV"],
    "Televisores > NanoCell": ["Electro", "TV"],
    "Televisores > UHD TV": ["Electro", "TV"],
    "Televisores > Smart TV": ["Electro", "TV"],
    "Televisores > Accesorios": ["Accesorios", ""],
    "Pantallas Comerciales": ["Electro", "TV"],
    "Monitores": ["Electro", "Tecnología"],
    "Electrodomésticos > Refrigeradores": ["Electro", "Heladeras"],
    "Electrodomésticos > Microondas": ["Electro", "Cocinas"],
    "Electrodomésticos > Lavado > Lavarropas": ["Electro", "Lavarropas"],
    "Electrodomésticos > Lavado > Lavasecarropas": ["Electro", "Lavarropas"],
    "Electrodomésticos > Lavado > WashTower": ["Electro", "Lavarropas"],
    "Electrodomésticos > Accesorios": ["Accesorios", ""],
    "Audio > Barras de sonido": ["Electro", "Audio"],
    "Audio > Minicomponentes": ["Electro", "Audio"],
    "Audio > Parlantes Bluetooth": ["Electro", "Audio"],
    "Audio > Torres de sonido": ["Electro", "Audio"],
    "Audio > Accesorios": ["Accesorios", ""],
}

def clasificar(cat):
    if cat in MAPEO: return MAPEO[cat]
    c = cat.lower()
    if "televisor" in c or "pantalla" in c: return ["Electro", "TV"]
    if "audio" in c: return ["Electro", "Audio"]
    if "refriger" in c: return ["Electro", "Heladeras"]
    if "lavado" in c: return ["Electro", "Lavarropas"]
    if "monitor" in c: return ["Electro", "Tecnología"]
...
def _handle(code): return "lg-" + re.sub(r'[^a-z0-9]+','-', code.lower()).strip('-')
...
def obtener():
    ...
    if not productos:
        raise RuntimeError("LTienda: feed vacio")
    ...
    for pcode, variantes in productos.items():
        ...
        for i,v in enumerate(variantes):
            ...
            if i==0:
                tg=", ".join([t for t in [sub,"LG"]+(["stock-verificado"] if hay_stock else []) if t])
                fila.update({"Handle":h,"Title":title,"Body HTML":body,"Vendor":"LG",
...
if __name__=="__main__":
    filas,n=obtener()
    print(f"LTienda: {n} productos, {len(filas)} filas")
